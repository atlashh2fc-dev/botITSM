...
import { ForumLogo, SondaLogo } from "@/components/shared/BrandMark";
import type { TenantId } from "@/lib/tenant/hosts";

export default function LandingPage({ tenantId }: { tenantId: TenantId }) {
  const router = useRouter();
  const isForum = tenantId === "forum";
  const supportBrand = isForum ? "Forum" : "SONDA";
  ...
  return (
    ...
    >
      ...
      <div style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: 12 }}>
        {isForum ? <ForumLogo width={140} /> : <SondaLogo width={140} />}
        ...
        >
          ...
          <div>
            ...
            <p style={{ margin: "10px 0 0", fontSize: 14, color: "#5A9668", lineHeight: 1.5 }}>
              IA para técnicos en terreno. Fotografía una falla y obtén diagnóstico con procedimientos {supportBrand}.
            </p>
          </div>
      ...
      <p style={{ color: "#1E2E3D", fontSize: 12, margin: 0, textAlign: "center" }}>
        {supportBrand} ITSM · Portal interno
      </p>
    </main>
  );
}
